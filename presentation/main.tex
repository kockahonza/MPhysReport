...
% de Gennes style theories and their problems
...
% My work 3D
\begin{frame}[fragile]{What is \EE\ in 3D}
    \newrefsection
    \vspace{-\fill}
    \begin{equation*}
        \du{E} \overset{\text{how?}}{\sim} \psi(\su{N}\su{N} - \frac{\du{\delta}}{d})
    \end{equation*}
    \vfill

    \begin{itemize}
        \item Treat \EE\ as general complex tensor
        \item Need arrive at the form above
        \item Inspiration from real symmetric tensors leads us to diagonalization
        \item Require $\du{E}$ be unitarily diagonalizable (\mul{normal}), \mul{\smash{symmetric}} and \mul{traceless}
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{What is \EE\ in 3D}
    \newrefsection
    Normality and tracelessness give
    \begin{equation*}
    ...
    where the $\lambda_i$ are eigenvalues of \EE\ and $\du{U}$ some unitary matrix
\end{frame}

\begin{frame}[fragile]{Our constraints lead to biaxial \EE\ in 3D}
    \newrefsection
    Also using symmetry this leads to
    \begin{equation*}
...
% Dynamics and F
\begin{frame}[fragile]{Evolve \EE\ in time via local free energy minimization}
    \newrefsection
    \begin{itemize}
        \item \EE\ is a Ginzburg-Landau theory
    ...
    \vspace{-1.5em}
    \begin{itemize}
        \extra{\item Model A like dynamics using Wirtinger derivatives}
        \extra{\item Plus Lagrange multipliers to enforce constraints}
    \end{itemize}
\end{frame}
