...
\section{New Perspectives on \EE\ in 3D}\label{sec:npE}
    ...
    And the motivation for adopting a traceless tensor and (so adding the $-\frac{\sdu{\delta}}{d}$ to the necessary $\su{N}\su{N}$) in \cref{eq:Eua_def} comes down to simplifying the bulk free energy density form (as has been noted in \cref{sec:Ei_F}) which could otherwise have terms of $E_{ii}=\Tr(\du{E})$.
...
\section{Dynamics of constrained \EE}\label{sec:dcE_Edyn2}
    ...
    \end{equation}
    which however does not in any way account for the constraints on \EE.
    ...
    Or in numerical simulations, which are the goal for us in this thesis, where we approximate the integral by discrete steps and there are various sources of small numerical errors.
    ...
    Finally, instead of analytically solving for the Lagrange multipliers $\lambda_i$ like Paget et al and Mihajlovic, we employ a soft constraint\cite{sadatiHardVsSoft2011,meseguerChapterSoftConstraints2006}.
...
\section{Free Energy and Projection Operators}
    ...
    \subsection{Expressing \PP\ in terms of \EE}
        ...
        Now, there is a major catch here of having to take the complex square root which is not a uniquely defined function, however we shall show later that this can be resolved.
