...
The magnitude of $\psi$ describes how smectic the system is at a particular point and $\su{N}$ is the layer normal of the dominant wave.
What is meant by that is that because we also have $\phi$ (the phase of $\psi$) varying, this may change the layering to not be precisely that described by \NN.
There is a degree of redundancy between the two as we need both.
We need an explicit \NN\ so that we can account for the layering symmetry, and we need an explicit $\phi$ to account for layer compressions and dilations.
This means that technically the precise layer normal may differ from \NN\ though we do not expect that to play a role and it does not in our simulations.
Specifically, this will not be an issue as long as $\phi$ varies slowly compared to \NN\ in the directions perpendicular to \NN.
